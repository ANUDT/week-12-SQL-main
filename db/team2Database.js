  ...
  getUnits() {
    return new Promise((resolve, reject) => {
      this.db.query('SELECT * FROM department;', (err, results) => {
        ...
        console.log("DB Rows:", results.rows)
...
addDepartment(department) {
 return new Promise((resolve, reject) => {
  ...
  addPosition(position) {
    return new Promise((resolve, reject) => {
      const { title, salary, department_id } = position;
      this.db.query(
        'INSERT INTO position (title, salary, department_id) VALUES ($1, $2, $3) RETURNING *',
        [title, salary, department_id],
        (err, results) => {
          if (err) {
            reject(err);
          } else {
            resolve(`Position ${title} added successfully`);
          }
        }
      );
    });
  ...
  addWorker(worker) {
    return new Promise((resolve, reject) => {
      ...
      this.db.query(
        'INSERT INTO worker (first_nm, last_nm, position_id, manager_id) VALUES ($1, $2, $3, $4) RETURNING *',
        [first_nm, last_nm, position_id, manager_id],
        (err, results) => {
          if (err) {
            reject(err);
          } else {
            const addedWorker = results.rows[0];
            resolve(`${addedWorker.first_nm} ${addedWorker.last_nm} added successfully with ID: ${addedWorker.id}`);
          }
        }
      );
    });
